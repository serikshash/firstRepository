...
class MySQLPrivSupRepository implements PrivSupRepositoryInterface{
 	public function getPrivSupList(){
 		...
 		$privSupA = array();
 		$query = 'SELECT id, first_name, second_name, last_name, reg_number FROM privat_supplier';
 ...
 public function getPrivSupByID($id){
 	...
 	mysqli_close($conn);
 	if ($privSup == NULL){
 	throw new Exception("object whith this id is missing");
 	}
 	return $privSup;
 }
 public function create($privSup) {
 	...
 	$query = "INSERT INTO privat_supplier(id, first_name, second_name, last_name, reg_number) VALUES ({$id}, '{$FName}', {$SName}, '{$LName}', '{$RegNum}')";
 	$result = mysqli_query($conn, $query);
 	if (!$result) 	{
 ...
 public function update($privSup){
 	...
 	$LName = $privSup->getLName();
 	$RegNum = $privSup->getRegNum();
	$query = "UPDATE privat_supplier SET first_name = '{$FName}', second_name = {$SName},last_name = '{$LName}', reg_number = '{$RegNum}' WHERE id = {$id}";
 	$result = mysqli_query($conn, $query);
 	if (!$result) 	{
 		throw new Exception(mysqli_error($conn));
 	}
...
public function delete($id){
 	$conn = MySQLConnectionUtil::getConnection();
 	$query = "DELETE FROM privat_supplier WHERE id = {$id}";
 	$result = mysqli_query($conn, $query);
	if (!$result) 	{
 		throw new Exception(mysqli_error($conn));
 	}
