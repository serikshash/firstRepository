...
class MySQLContractRepository implements ContractRepositoryInterface
{
	...
	public function getContractListByProduct()
	{
		$conn = MySQLConnectionUtil::getConnection();
		$contracts = array();
		$query = 'SELECT DISTINCT contract.number,contract.agreed,contract.supplier,contract.title,contract.note  FROM product,contract WHERE product.contract=contract.number';
		$result = mysqli_query($conn, $query);
		while ($row = mysqli_fetch_assoc($result))
			{
				$contract = new Contract($row['number'], $row['agreed'], $row['supplier'], $row['title'], $row['note']);
